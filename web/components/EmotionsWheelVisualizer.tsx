...
export const EMOTION_METADATA: EmotionMeta[] = [
...
export const PRESETS: Array<{ label: string; state: Partial<Record<PrimaryEmotion, number>> }> = [
...
export interface PlutchikWheelGraphicProps {
  emotionsState?: Record<PrimaryEmotion, number>;
  emotionEngine?: EmotionsOrchestrator;
  derivedEmotion?: PlutchikEmotion;
  size?: number;
  className?: string;
}

export const PlutchikWheelGraphic: React.FC<PlutchikWheelGraphicProps> = ({
  emotionsState,
  emotionEngine,
  derivedEmotion,
  size = 180,
  ...
  const currentState = emotionsState || (emotionEngine ? emotionEngine.getState() : {
    joy: 0, trust: 0, fear: 0, surprise: 0, sadness: 0, disgust: 0, anger: 0, anticipation: 0,
  });
  const currentEmotion = derivedEmotion || (emotionEngine ? emotionEngine.getOverallEmotion() : 'Love');

  const center = size / 2;
  const radius = (size / 180) * 72;
  const innerRadius = (size / 180) * 26;

  return (
    <div
      className={`wheel-graphic-box ${className}`.trim()}
    ...
    >
      ...
      >
        ...
        {EMOTION_METADATA.map((meta) => {
          const intensity = currentState[meta.key] ?? 0;
...
export interface EmotionStatusBadgeProps {
  derivedEmotion: PlutchikEmotion;
  mappedFramesText?: string;
  responseType?: ResponseType;
  onChangeResponseType?: (newType: ResponseType) => void;
  showResponseTypeSelect?: boolean;
}

export const EmotionStatusBadge: React.FC<EmotionStatusBadgeProps> = ({
  derivedEmotion,
  mappedFramesText,
  ...
  return (
    <div className="wheel-status-badge">
      ...
      {showResponseTypeSelect && onChangeResponseType && (
        <div className="response-type-wrapper">
          <Select
            ...
            onChange={(val) => onChangeResponseType(val as ResponseType)}
          />
        </div>
      )}
    </div>
...
export interface EmotionPresetMixersProps {
  onApplyPreset: (presetState: Partial<Record<PrimaryEmotion, number>>) => void;
}

export const EmotionPresetMixers: React.FC<EmotionPresetMixersProps> = ({ onApplyPreset }) => {
  return (
...
export interface EmotionIntensityTunersProps {
  emotionsState: Record<PrimaryEmotion, number>;
  onSliderChange?: (emotionKey: PrimaryEmotion, value: number) => void;
  readOnly?: boolean;
  title?: string;
}

export const EmotionIntensityTuners: React.FC<EmotionIntensityTunersProps> = ({
  emotionsState,
  onSliderChange,
  readOnly = false,
  title,
    ...
    <div className="volume-tuners-section">
      <div className="tuners-header">
        <span className="tuners-title">
          {title || (readOnly ? 'Emotion Intensity (Live Levels)' : 'Emotion Intensity Tuners (0% - 100%)')}
        </span>
      </div>
            ...
            <div key={meta.key} className="tuner-card" style={{ '--tuner-color': meta.color } as React.CSSProperties}>
              ...
              <div className="tuner-controls">
                <input
                  ...
                  value={intensity}
                  disabled={readOnly}
                  onChange={(e) => !readOnly && onSliderChange?.(meta.key, parseFloat(e.target.value))}
                />
...
export interface EmotionsWheelVisualizerProps {
  emotionEngine: EmotionsOrchestrator;
...
export const EmotionsWheelVisualizer: React.FC<EmotionsWheelVisualizerProps> = ({
  ...
  return (
    <PanelSection
      ...
      bgVariant="actions"
    >
      {/* Visual Plutchik Wheel & Status Badge */}
      <div className="emotions-wheel-top-container">
        ...
        <EmotionStatusBadge
          derivedEmotion={derivedEmotion}
          mappedFramesText={mappedFramesText}
          responseType={responseType}
      ...
      <EmotionPresetMixers onApplyPreset={applyPreset} />

      {/* Volume Tuner Console */}
      <EmotionIntensityTuners
        emotionsState={emotionsState}
